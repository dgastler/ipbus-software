...
#ifndef _uiouhal_ProtocolUIOPCI_hpp_
#define _uiouhal_ProtocolUIOPCI_hpp_
...
namespace uhal {
  ...
  namespace exception
  {
    ...
    UHAL_DEFINE_EXCEPTION_CLASS ( BadUIOPCIDevice , "Exception class to handle the case where uio device cannot be opened." )
    UHAL_DEFINE_EXCEPTION_CLASS ( UnimplementedFunctionUIOPCI , "Exception class to handle the case where an unimplemented function is called." )
    //    UHAL_DEFINE_EXCEPTION_CLASS ( UIOBusError , "Exception class for when an axi transaction causes a BUS_ERROR." )
  }

class UIOPCI : public ClientInterface {
  public:
    UIOPCI (
